...
CREATE TABLE t_credit_quota (
    ...
    available_quota DECIMAL(18,2) DEFAULT 0.00 COMMENT '可用额度(手动维护：总额度-已用额度-冻结额度)',
...
CREATE TABLE t_quota_usage_detail (
    ...
    transaction_type ENUM('OCCUPY', 'RELEASE', 'FREEZE', 'UNFREEZE', 'ADJUST', 'PRE_OCCUPY', 'CANCEL_PRE_OCCUPY') NOT NULL COMMENT '交易类型',
...
CREATE TABLE t_risk_warning (
    ...
    warning_status ENUM('UNHANDLED', 'HANDLING', 'HANDLED', 'IGNORED', 'RESOLVED') DEFAULT 'UNHANDLED' COMMENT '预警状态',
...
CREATE TABLE t_approval_process (
    ...
    process_status ENUM('RUNNING', 'COMPLETED', 'TERMINATED', 'CANCELLED') DEFAULT 'RUNNING' COMMENT '流程状态',
...
CREATE TABLE t_approval_node (
    ...
    assignee_type ENUM('USER', 'ROLE', 'DEPARTMENT', 'POSITION') COMMENT '处理人类型',
    assignee_id VARCHAR(64) COMMENT '处理人ID',
    assignee_name VARCHAR(100) COMMENT '处理人姓名',
    node_status ENUM('PENDING', 'PROCESSING', 'COMPLETED', 'REJECTED', 'CANCELLED') DEFAULT 'PENDING' COMMENT '节点状态',
    approve_result ENUM('PENDING', 'APPROVED', 'REJECTED', 'TRANSFERRED', 'DELEGATED') DEFAULT 'PENDING' COMMENT '审批结果',
...
CREATE TABLE t_sys_param_config (
    ...
    data_type ENUM('STRING', 'NUMBER', 'DATE', 'JSON', 'BOOLEAN') DEFAULT 'STRING' COMMENT '数据类型',
    status ENUM('ACTIVE', 'INACTIVE') DEFAULT 'ACTIVE' COMMENT '状态',
...
-- 初始化系统参数配置
INSERT INTO t_sys_param_config (param_id, param_group, param_code, param_name, param_value, param_desc, status, created_by) VALUES
('QUOTA_CALC_METHOD', 'RISK_CONTROL', 'quota_calculation_method', '额度测算方法', 'BASED_ON_ASSETS', '额度测算方法：基于资产、现金流等', 'ACTIVE', 'SYSTEM'),
('WARN_THRESHOLD_LOW', 'RISK_CONTROL', 'warning_threshold_low', '低风险阈值', '0.6', '风险指标低于此值为低风险', 'ACTIVE', 'SYSTEM'),
('WARN_THRESHOLD_MEDIUM', 'RISK_CONTROL', 'warning_threshold_medium', '中风险阈值', '0.75', '风险指标在此区间为中风险', 'ACTIVE', 'SYSTEM'),
('WARN_THRESHOLD_HIGH', 'RISK_CONTROL', 'warning_threshold_high', '高风险阈值', '0.9', '风险指标高于此值为高风险', 'ACTIVE', 'SYSTEM'),
('QUOTA_EXPIRY_ALERT_DAYS', 'RISK_CONTROL', 'quota_expiry_alert_days', '额度到期提醒天数', '30', '额度到期前多少天开始提醒', 'ACTIVE', 'SYSTEM'),
('GROUP_QUOTA_RATIO_LIMIT', 'RISK_CONTROL', 'group_quota_ratio_limit', '集团额度分配比例限制', '0.8', '集团额度分配给子公司的最高比例', 'ACTIVE', 'SYSTEM'),
('DEFAULT_QUOTA_CURRENCY', 'QUOTA_CONFIG', 'default_currency', '默认币种', 'CNY', '系统默认使用的币种', 'ACTIVE', 'SYSTEM'),
('QUOTA_ADJUST_MIN_AMOUNT', 'QUOTA_CONFIG', 'quota_adjust_min_amount', '额度调整最小金额', '1000.00', '额度调整的最小金额单位', 'ACTIVE', 'SYSTEM'),
('QUOTA_FREEZE_DAYS_MAX', 'QUOTA_CONFIG', 'quota_freeze_max_days', '额度冻结最长期限', '365', '额度冻结的最大天数', 'ACTIVE', 'SYSTEM');

-- 初始化额度类型数据
INSERT INTO t_sys_param_config (param_id, param_group, param_code, param_name, param_value, param_desc, status, created_by) VALUES
('QUOTA_TYPE_1', 'QUOTA_CONFIG', 'CREDIT_LINE', '综合授信额度', '1', '综合授信额度类型', 'ACTIVE', 'SYSTEM'),
('QUOTA_TYPE_2', 'QUOTA_CONFIG', 'WORKING_CAPITAL', '流动资金贷款额度', '2', '流动资金贷款专项额度', 'ACTIVE', 'SYSTEM'),
('QUOTA_TYPE_3', 'QUOTA_CONFIG', 'FIXED_ASSET_LOAN', '固定资产贷款额度', '3', '固定资产贷款专项额度', 'ACTIVE', 'SYSTEM'),
('QUOTA_TYPE_4', 'QUOTA_CONFIG', 'TRADE_FINANCE', '贸易融资额度', '4', '贸易融资专项额度', 'ACTIVE', 'SYSTEM'),
('QUOTA_TYPE_5', 'QUOTA_CONFIG', 'GUARANTEE_QUOTA', '担保额度', '5', '基于抵押物的担保额度', 'ACTIVE', 'SYSTEM'),
('QUOTA_TYPE_6', 'QUOTA_CONFIG', 'BILL_QUOTA', '票据额度', '6', '银行承兑汇票等票据类额度', 'ACTIVE', 'SYSTEM'),
('QUOTA_TYPE_7', 'QUOTA_CONFIG', 'INTERNATIONAL_TRADE', '国际贸易融资额度', '7', '信用证、保函等国际贸易融资额度', 'ACTIVE', 'SYSTEM');

-- 初始化客户等级数据
INSERT INTO t_sys_param_config (param_id, param_group, param_code, param_name, param_value, param_desc, status, created_by) VALUES
('CUST_LEVEL_S', 'CUSTOMER_CONFIG', 'CUSTOMER_LEVEL_S', 'S级客户', 'S', '战略级客户', 'ACTIVE', 'SYSTEM'),
('CUST_LEVEL_A', 'CUSTOMER_CONFIG', 'CUSTOMER_LEVEL_A', 'A级客户', 'A', '优质客户', 'ACTIVE', 'SYSTEM'),
('CUST_LEVEL_B', 'CUSTOMER_CONFIG', 'CUSTOMER_LEVEL_B', 'B级客户', 'B', '一般客户', 'ACTIVE', 'SYSTEM'),
('CUST_LEVEL_C', 'CUSTOMER_CONFIG', 'CUSTOMER_LEVEL_C', 'C级客户', 'C', '关注客户', 'ACTIVE', 'SYSTEM'),
('CUST_LEVEL_D', 'CUSTOMER_CONFIG', 'CUSTOMER_LEVEL_D', 'D级客户', 'D', '风险客户', 'ACTIVE', 'SYSTEM');

-- 初始化审批流程节点类型
INSERT INTO t_sys_param_config (param_id, param_group, param_code, param_name, param_value, param_desc, status, created_by) VALUES
('NODE_TYPE_START', 'APPROVAL_CONFIG', 'START_NODE', '开始节点', 'START', '审批流程开始节点', 'ACTIVE', 'SYSTEM'),
('NODE_TYPE_APPROVAL', 'APPROVAL_CONFIG', 'APPROVAL_NODE', '审批节点', 'APPROVAL', '普通审批节点', 'ACTIVE', 'SYSTEM'),
('NODE_TYPE_SIGN', 'APPROVAL_CONFIG', 'SIGN_NODE', '会签节点', 'SIGN', '需要多人同时审批的节点', 'ACTIVE', 'SYSTEM'),
('NODE_TYPE_CONDITION', 'APPROVAL_CONFIG', 'CONDITION_NODE', '条件节点', 'CONDITION', '根据条件判断流向的节点', 'ACTIVE', 'SYSTEM'),
('NODE_TYPE_END', 'APPROVAL_CONFIG', 'END_NODE', '结束节点', 'END', '审批流程结束节点', 'ACTIVE', 'SYSTEM');